/**
 * island-detection — 지현규 슬라이스 기반 island(미지지 영역) 검출 알고리즘 (v2 이식).
 *
 * 원본: frontend/src/utils/slice-detection.utils.ts (643줄, 지현규 브랜치) 전체 이식.
 * 로직 무변경 — 순수 이동 + v2 트리 재배치.
 *
 * 유일한 어댑터: 원본이 `./support.utils` 에서 import 하던 TriInfo 타입을
 * 같은 dental 모듈로 함께 옮긴 `./dental-support` 에서 import 하도록 경로만 변경.
 * (TriInfo 정의 자체는 원본과 동일 — 수치 동작 무변경.)
 */
import { Vector3 } from '@babylonjs/core';
import type { TriInfo } from './dental-support';
