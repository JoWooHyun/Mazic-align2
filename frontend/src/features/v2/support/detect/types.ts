...
export interface LayerGraphParams {
  ...
  plateGapMm: number;
  /**
   * 오버행 검출 기준각 θ (deg). 설계 3-1b. 모델 표면(층 폴리곤 가장자리)이
   * 이보다 더 누우면(수평에 가까우면) 오버행으로 판정. CHITUBOX 기본 30°.
   *
   *   **실적용 (S-4b-2e).** 층-팽창 방식으로 판정한다: 지지 반경
   *   r = layerHeightMm / tan(θ) 를 두고, 샘플점이 아래층 폴리곤 안이거나
   *   그 경계에서 r 이내면 "지지됨", r 보다 멀리 튀어나오면 오버행.
   *   표면 기울기 α 인 면의 층당 전진량 lh/tan(α) 와 r 의 대소가 α<θ 판정과
   *   같기 때문이다. θ=30°, lh=0.05 → r≈0.0866mm (45° 벽은 지지, 20° 완경사는
   *   오버행). 판정·클램프 상세는 layer-graph.ts 의 supportReachMm 참고.
   */
