 ...
 *
 * **예외 3: 수직 이동(`ty`)** (B-18). 리드가 타 슬라이서 실물 화면과 대조해 확정한
 * 정책 변경이다 — "수직이동은 **서포터 달린 상태로 올라갔다 내려오더라**.
 * **서포터랑 stl 이랑 아예 다른 객체 취급**이야." 실물에서는 모델이 공중에 떠 있고
 * (Z=6.71) 서포트가 **바닥에서 모델까지** 늘어나 있었다. 즉 서포트는 모델에 종속된
 * 게 아니라 **플레이트에 서 있는 독립 구조물**이고, 모델은 그 위에 얹혀 있다.
 * 모델이 오르내리면 서포트는 바닥에 붙은 채 **기둥 길이만 변하면 된다** → 삭제할
 * 이유가 없다.
 *   ⚠️ 이 예외는 **혼자 성립하지 않는다**. 판정만 바꾸고 조립을 그대로 두면 발이
 *   모델을 따라 떠오른다(base 가 stl-local 이라). 짝이 되는 변경이
 *   `support/assemble-core.ts` 의 `resolveRedesignBaseY` — 기둥 발을 플레이트
 *   (world Y=0)에 고정한다. 둘은 **반드시 함께** 유지할 것.
 *
 * 스케일은 `sy` 는 물론 균일 스케일도 접점 위치·접지가 달라지므로 전부 무효다.
 * (스케일은 모델 자체의 크기가 변해 접점 XZ 까지 흩어지므로 기둥 길이 조정만으로는
 * 복구되지 않는다 — 수직 이동과 다른 경우다.)
 *
 * 따라서 **기둥 기울기 각도**·sx/sy/sz(스케일) 중 하나라도 실제로 바뀌면
 * 무효(false), tx/tz(수평 이동)·수직축 회전·ty(수직 이동)만이면 유효(true).
 *
...
export function transformKeepsRedesignValid(
  ...
  // 스케일은 단위가 있는 성분이라 지금까지처럼 성분 비교 그대로.
  //   ty 항은 B-18 에서 제거됐다 — 수직 이동은 기둥 길이로 흡수되므로 무효가
  //   아니다(위 예외 3). 되돌리려면 assemble-core 의 플레이트 고정도 같이 되돌릴 것.
  return !(
    tilted ||
